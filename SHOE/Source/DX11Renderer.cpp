...
void DX11Renderer::PreResize() {
	this->backBufferRTV.Reset();
	this->depthBufferDSV.Reset();
}

void DX11Renderer::PostResize(unsigned int windowHeight,
	unsigned int windowWidth,
	Microsoft::WRL::ComPtr<ID3D11RenderTargetView> backBufferRTV,
	Microsoft::WRL::ComPtr<ID3D11DepthStencilView> depthBufferDSV
)
...
// ------------------------------------------------------------------------
void DX11Renderer::DrawPointLights(std::shared_ptr<Camera> cam)
{
...
void DX11Renderer::RenderDepths(std::shared_ptr<Camera> sourceCam, MiscEffectSRVTypes type) {
...
/// </summary>
void DX11Renderer::RenderShadows() {
	shadowDSVArray.clear();
	shadowProjMatArray.clear();
	shadowViewMatArray.clear();
...
void DX11Renderer::RenderColliders(std::shared_ptr<Camera> cam)
{
...
void DX11Renderer::RenderMeshBounds(std::shared_ptr<Camera> cam)
{
	// Set the shaders to be used
	basicVS->SetShader();
	solidColorPS->SetShader();
...
void DX11Renderer::RenderSelectedHighlight(std::shared_ptr<Camera> cam, EngineState engineState)
{
...
void DX11Renderer::Draw(std::shared_ptr<Camera> cam, EngineState engineState) {
	RenderShadows();
...
HRESULT DX11Renderer::WriteFrame(Microsoft::WRL::ComPtr<IMFSinkWriter> sinkWriter, DWORD streamIndex, const long long int& timeStamp, FileRenderData* RenderParameters) {
...
HRESULT DX11Renderer::InitializeFileSinkWriter(OUT Microsoft::WRL::ComPtr<IMFSinkWriter>* sinkWriterOut, DWORD* pStreamIndex, FileRenderData* RenderParameters) {
	*pStreamIndex = NULL;
